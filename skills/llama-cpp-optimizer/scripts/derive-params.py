...
def load_json_input(arg: str) -> dict[str, Any]:
    ...
    try:
        ...
        print(f"Error: file not found and not valid JSON: {arg[:80]}...", file=sys.stderr)
...
def derive_params(system: SystemInfo, model: ModelInfo) -> DerivedParams:
    ...
    params.threads = system.cpu_cores_logical or 8
    ...
    for gpu in system.gpus:
        vram_total_mb = max(vram_total_mb, gpu.get("vram_total_mb", 0))
        vram_free_mb = max(vram_free_mb, gpu.get("vram_free_mb", 0))
    ...
    hd = model.head_dim or (model.hidden_size // (model.num_attention_heads or 16) if model.hidden_size else 128)
    ...
    if model.is_moe:
        ...
        active_expert_gb = model_file_gb * (1 - dense_ratio) * (
            model.num_experts_per_tok / model.num_experts
        ) if model.num_experts > 0 else 0
        ...
        if vram_needed_moe < vram_free_gb:
            ...
            layer_gb = dense_gb / model.num_hidden_layers if model.num_hidden_layers else 0.5
        ...
        # Dense model
        layer_gb = model_file_gb / model.num_hidden_layers if model.num_hidden_layers else 0.5
        available = vram_free_gb - kv_cache_gb - 0.5  # overhead
...
def main() -> None:
    ...
    if args.system:
        ...
        system = SystemInfo(**{k: system_data.get(k, v) for k, v in asdict(SystemInfo()).items()})
    else:
        system = detect_system()

    # Load model info
    model_data = load_json_input(args.model)
    model = ModelInfo(**{k: model_data.get(k, v) for k, v in asdict(ModelInfo()).items()})
